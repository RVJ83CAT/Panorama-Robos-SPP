...
function initMap() {
    ...
    if (typeof L === 'undefined') {
        const errorMsg = 'Leaflet no está cargado. Verifique que lib/leaflet.min.js esté presente y no esté corrupto.';
        logError(errorMsg);
        document.getElementById('map-loading').textContent = 'Error: No se pudo cargar Leaflet. Asegúrese de que lib/leaflet.min.js esté en la carpeta correcta.';
        return false;
    }
    try {
...
function changeMapProvider() {
    ...
    if (providers[provider]) {
        ...
        currentLayer = providers[provider].addTo(map).on('tileerror', () => {
            logError('Error al cargar tiles de ' + provider + '. Revirtiendo a OpenStreetMap.');
            if (provider !== 'osm') {
                document.getElementById('mapProvider').value = 'osm';
                changeMapProvider();
            }
        });
        map.invalidateSize();
...
function addOrUpdateMarker(data, index = null, saveFile = true) {
    ...
    if (data.LATITUD !== 0 && data.LONGITUD !== 0 && !isNaN(data.LATITUD) && !isNaN(data.LONGITUD)) {
        const iconUrl = data.DELITO && delitoIcons[data.DELITO] ? delitoIcons[data.DELITO].options.iconUrl : './iconos/default.png';
        const icon = L.icon({
            iconUrl,
            iconSize: [32, 32],
            iconAnchor: [16, 32],
...
function updateFilterOptions() {
    ...
    const rangoHoraSelect = document.getElementById('rangoHoraFilter');
    const cuadranteSelect = document.getElementById('cuadranteFilter');
    ...
    rangoHoraSelect.innerHTML = '<option value="">Todos</option>';
    rangosHora.forEach(rango => {
        const option = document.createElement('option');
        option.value = rango;
        option.textContent = rango;
        rangoHoraSelect.appendChild(option);
    });

    cuadranteSelect.innerHTML = '<option value="">Todos</option>';
    cuadrantes.forEach(cuadrante => {
        const option = document.createElement('option');
        option.value = cuadrante;
        option.textContent = cuadrante;
        cuadranteSelect.appendChild(option);
    });

    console.log('Opciones de filtros actualizadas:', { delitos, rangosHora, cuadrantes });
}
...
function updateTable(data, page = 1) {
    ...
    if (!data || data.length === 0) {
        tbody.innerHTML = '<tr><td colspan="12">No hay datos para mostrar</td></tr>';
...
function saveToXLSX() {
    if (typeof XLSX === 'undefined') {
        logError('SheetJS no está cargado. Verifique que lib/xlsx.full.min.js esté presente.');
        return;
    }
    try {
...
function processExcelFile(file) {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (!headers.every((h, i) => h === expectedNormalized[i])) {
                console.warn('Los encabezados del Excel no coinciden con los esperados:', headers, expectedNormalized);
                errorMessage.textContent = 'Advertencia: Los encabezados del archivo no coinciden con los esperados. Los datos podrían no cargarse correctamente.';
            }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('filterForm').addEventListener('submit', function(e) {
        ...
        const rangoHora = document.getElementById('rangoHoraFilter').value;
        const cuadrante = document.getElementById('cuadranteFilter').value;
        ...
        const filteredData = allData.filter((data, index) => {
            ...
            const matchDate = (!startDate || !dataDate || dataDate >= startDate) && (!endDate || !dataDate || dataDate <= (endDate ? new Date(endDate.setHours(23, 59, 59, 999)) : endDate));
            const matchDelito = !delito || data.DELITO === delito;
            const matchRangoHora = !rangoHora || data['RANGO HORA'] === rangoHora;
            const matchCuadrante = !cuadrante || data.CUADRANTE === cuadrante;

            if (matchDate && matchDelito && matchRangoHora && matchCuadrante) {
                addOrUpdateMarker(data, index, false);
                return true;
            }
            return false;
        });
